...
fn extract_coordinate(temp_coordinate: &str) -> (i32, i32) {
    ...
    let output_x = loop {
        ...
        let coordinate_add_x = temp_coordinate
            .chars()
            .nth(x_index + 1 + *loopcounter_x_pointer)
            .unwrap();
        ...
        println!(
            "PRINTED FINAL STRING AFTER CONVERSION{:?}",
            convert_string_toi32(x_finalstring.to_string())
        );
    ...
    let output_y = loop {
        ...
        let coordinate_add_y = temp_coordinate
            .chars()
            .nth(y_index + 1 + *loopcounter_y_pointer)
            .unwrap();
        ...
        println!(
            "PRINTED FINAL STRING AFTER CONVERSION{:?}",
            convert_string_toi32(y_finalstring.to_string())
        );
